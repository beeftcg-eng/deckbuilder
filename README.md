...
- **Tournament record from Pairings**: link a deck to [Pairings](https://pairings-coral.vercel.app), the TCG tournament tracker, and its deck view shows the record you've logged with it there (wins-losses-draws, win rate, number of events, the opponents it beats and loses to most, and each event with its placement and record), and My Decks shows each deck's W-L. Results refresh when you come back to the window (at most once a minute) or with ↻. In Pairings, open **Decks → Import from Beef's Brewhouse** (log in there with this app's Pawmodoro account) to import a deck; then here, press **🏆 Tournament record: connect Pairings** in the deck view and log in with your Pairings account. It only reads your results. Pairings' **Open in Brewhouse ↗** opens the deck in the phone app (`?deck=<id>`). To make that work, every save also stores a short readable `summary` on the deck (legend/leader, colors, card count, format), since Pairings can't read card ids.
